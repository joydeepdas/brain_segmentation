...
class SegmentationModel(object):
    def __init__(self, n_epoch=6, n_chan=4, batch_size=128, loaded_model=False, architecture='single', w_reg=0.01, n_filters=[64,128,128,128], k_dims = [7,5,5,3], activation = 'relu'):
        '''
    ...
    def compile_model(self):
        '''
        compiles standard single model with 4 convolutional/max-pooling layers.
        '''
        ...
        model.add(Convolution2D(64, kernel_size=(3, 3), padding="same", data_format='channels_first', input_shape = (4, 33,33), kernel_initializer = 'glorot_normal', bias_initializer=constant(0.1) ))
    ...
    def fit_model(self, X_train, y_train, X5_train = None, save=True):
        ...
        if self.architecture == 'dual':
            ...
            self.model_comp.fit(X_train, Y_train, batch_size=self.batch_size, nb_epoch=self.n_epoch, validation_split=0.1,  verbose=1, callbacks=[checkpointer])
...
if __name__ == '__main__':
    # train_data = glob('n4_PNG/**')
    # patches = PatchLibrary((33,33), train_data, 500)
    # X,y = patches.make_training_patches()

    # model = SegmentationModel()
    # model.fit_model(X, y)
    # model.save_model('models/simple')

    #tests = glob('test_data/2_*')
    tests = glob('train_data/2_*')
    # print(tests)
    test_sort = sorted(tests, key= lambda x: int(x[13:-4]))
    # model = BasicModel(loaded_model=True)
    model = SegmentationModel(loaded_model=True)
    segmented_images = []
    # start from 30th image.
    slide_no=30
    for slice in test_sort[30:145]:
        segmented_slice=model.show_segmented_image(slice)
        segmented_images.append(segmented_slice)
        # segmented_slice= 255. * segmented_slice
        # io.imshow(segmented_slice)
        # plt.show()
        # print(segmented_slice.shape)
        # np.save('test_result/slices_npy/slice_{}.npy'.format(slide_no),segmented_slice)
        io.imsave('test_result/slice_{}.png'.format(slide_no), segmented_slice)    
        slide_no=slide_no+1
    np.save('test_result/final_result.npy',segmented_images)    
        
        
        
        # print(type(segmented_slice))
        # print(segmented_slice.shape)
        # io.imsave('test_result/slice_{}.png'.format(slide_no), segmented_slice)

        # cv2.imwrite('test_result/slice_{}.png'.format(slide_no), segmented_slice)
        # scipy.misc.imsave('test_result/outfile.jpg', segmented_slice)
        # slide_no=slide_no+1
